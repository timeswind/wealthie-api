...
exports.getAllUnclaimed = function (selectFields) {
    return List.find({public: true}, selectFields).populate('listBy', 'firstName').lean().exec()
};
...
exports.getLatest = function (limit, selectFields) {
  if (arguments.length === 1) {
    return List.find({}).sort({_id: -1}).limit(limit).populate('advisor', 'firstName lastName').lean().exec()
  } else {
    return List.find({}, selectFields).sort({_id: -1}).limit(limit).populate('advisor', 'firstName lastName').lean().exec()
  }
};
