...
exports.getLatest = function (limit, selectFields) {
  if (arguments.length === 1) {
    return List.find({}).limit(limit).populate('advisor', 'firstName lastName').lean().exec()
  } else {
    return List.find({}, selectFields).limit(limit).populate('advisor', 'firstName lastName').lean().exec()
  }
};
